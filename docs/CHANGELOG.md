...
## [v7.1.0] 2026-08-27 — SFT 关键帧面板 + 详情页 tab 切换 + 骨干句机制

### 核心变更

#### 新增

- **SFT 关键帧面板**(设计文档 `docs/superpowers/specs/2026-08-27-sft-keyframes-design.md`):SFT 标注详情卡内新增候选帧(10 帧均采,复用按需抽帧接口不落盘)与事件关键帧(chunk 级 2-5 帧)两行;hover 加/删、HTML5 拖拽排序、点击放大;增删改即时双写 `analysis/<stem>/关键帧/` 文件夹与 SFT 样本 JSON `keyframes` 字段;qwen3.8 智能挑选三路径——详情页手动、推理成功后自动、文件列表「批量关键帧」(与事件检测解耦,不动已核实文本标注)
- **详情页结果卡 tab 切换**:视频预览常驻,SFT 标注 / 分析报告 / 证据编辑单卡显示,选择持久化 localStorage
- **骨干句机制**:未检出事件手动勾选「检出」时,文本框段首自动插入该类骨干句(10 类模板,槽位=声明提及,chips 换值就地联动);`event_options.yaml` 事件 11 新增车道类型组(旧样本无此键不受影响)

#### 修复

- 取消勾选回收骨干句:未被人工改过自动移除恢复原文,chips 仅勾选时显示;被改过则保留并提示
- TokenText 陈旧显示:草稿被整体替换(重置/切换视频/保存重建)时不重算分段,继续输入会把旧文本写回草稿(假复活)
