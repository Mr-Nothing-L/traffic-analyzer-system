...
11 位二进制编码 `{bit_1_..._bit_11}`(位序=事件编号,位 9 为正常指示位:已分析且无事件检出时为 1,见 [ADR-0001](adr/0001-binary-encoding-bit-9-semantics.md))+ Markdown 报告 + SFT 样本(批量模式)。事件定义仍以 `config/event_categories.yaml` 为准。

## 实施状态(截至 2026-08-25,最终态)

- **P0 完成**:基线 commit `b241ae2`、codegraph 索引、kimi-code 调研、依赖安装。
- **P1 完成**:`agent/` 骨架落地(kosong vendored、llm provider、tool registry/scheduler、loop、permissions、sandbox),`npx vitest run` 104 个测试全绿(mock LLM)(历史快照;当前数量见 README)。
...
- **P5 部分完成(瘦身从简)**:pytest 809 个全绿、文档已更新(历史快照;当前数量见 README);旧 `web/chat/` 快速对话~~未删除~~**已删除**(对话统一,见下)。
