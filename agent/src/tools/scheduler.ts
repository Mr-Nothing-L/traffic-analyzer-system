/**
 * Same-batch tool call scheduler: tasks whose declared accesses do not
 * conflict run in parallel; conflicting tasks queue until the conflicting
 * predecessors finish. Result promises are handed out in add() order, so a
 * batch's results stay ordered like its tool calls.
 *
 * Ported from MoonshotAI/kimi-code (MIT)
 * packages/agent-core-v2/src/agent/toolExecutor/toolScheduler.ts.
 */
import { ToolAccesses } from './contract';
...
export class ToolScheduler<Result> {
  ...
  /** Add a whole batch; resolves with results in the same order as `tasks`. */
  runBatch(tasks: readonly ToolCallTask<Result>[]): Promise<Result[]> {
