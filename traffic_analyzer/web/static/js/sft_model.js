...
export const SFT_SKELETON_TEMPLATES = {
...
export function sanitizeFileAttrs(ev, raw) {
  const attrs = {};
...
export function missingRequired(ev, attrs) {
  const miss = [];
  evOptions(ev).forEach(g => {
...
export function parseSftDescription(desc, events) {
...
export function sftConclusionLines() {
...
export function buildSftRevision() {
  const d = state.sftDraft;
  const events = state.eventConfig || [];
  const sections = [];
  events.forEach(ev => {
...
export function initSftDraft(sft) {
  const events = state.eventConfig || [];
