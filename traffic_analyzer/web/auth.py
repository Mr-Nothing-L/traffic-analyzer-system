"""Optional env-configured session auth for the web UI.

Auth is fully OFF unless ``TRAFFIC_ANALYZER_USERS`` is set
(``zhangsan:pass1,lisi:pass2``); when off every request gets
``request.state.user = "local"`` and behavior is identical to before.
...
absent a random one is generated and appended to the project-root ``.env``
so sessions survive restarts. The middleware re-verifies signature, expiry
and that the request IP still equals ``login_ip``; unauthenticated page
requests get 302 → ``/login``, API requests 401.

[文件说明]
作用:可选的会话认证(env 配置账号,未配置则完全关闭,request.state.user='local')。
...
项目根 .env,30 天有效,httpOnly,sameSite=lax);middleware 校验签名+过期+
...
# 项目根(traffic_analyzer/web/auth.py → parents[2])。测试 monkeypatch 此常量。
...
class AuthConfig:
    ...
    def __init__(self, users: Dict[str, str], secret: Optional[str]) -> None:
        self.users = users
        self.secret = secret
        self.enabled = bool(users) and bool(secret)
...
def _load_or_create_secret() -> str:
    """Secret from env, then project-root .env, else generate + append to .env."""
